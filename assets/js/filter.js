...
filterButton.addEventListener('click', function(e){
            ...
            $.ajax({
                    ...
                    error: function( error){ 
                        console.log("Error in fecting data from API", error); 
                        // if api fails it will show error to user
                            },
                    success: function (response) {
                        singleProject(response.projects);
                         //it will send projects from api to next function
                    }
                });
                ...
                function singleProject(project){
                 project.forEach(element => {
                      if(id === element._id){
                          filter(element.issues) 
                          // sending issues and id of project to next funcion
                      }
                  });
                }
           ...
           for(let i = 0; i < authors.length; i++){ 
               ...
               if(authors[i].checked == true){
                   //checking if author is selected for filtering
                        ...
                        filterArray[authors[i].value] = 1;
                        //then only add it into map
                    }
               }
           };
